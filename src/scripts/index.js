...
$(function () {
  ...
  function LoadDashBoardData() {
    $.ajax({
      ...
      success: (users) => {
        var user = users.find(
          (user) => user.UserId == $("#Login-UserId").val()
        );
            ...
            tasks.map((task) => {
              $(`
                                    <div class="alert m-2 alert-success alert-dismissible">
                                      ...
                                      <button data-bs-toggle="modal" data-bs-target="#edit-task" value=${
                                        task.AppointmentId
                                      } id="btnEditTask" class="btn btn-warning bi bi-pen-fill">Edit Task</button>
                                      <button value=${
                                        task.AppointmentId
                                      } id="btnDeleteTask" class="btn btn-danger bi bi-trash-fill">Delete Task</button>
                                    </div>
                                 `).appendTo("main");
            });
          },
  ...
  $(document).on("click", "#btnSaveAppointment", () => {
    ...
    $.ajax({
      method: "put",
      url: `http://127.0.0.1:6600/update-task/${$(
        "#Edit_AppointmentId"
      ).val()}`,
      data: task,
      success: () => {
